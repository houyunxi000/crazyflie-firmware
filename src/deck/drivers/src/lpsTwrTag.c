...
#define basicAddr 0xE7E7E7E7E1 // 这里做了-修改
#define add2ID 0x0F
...
static void txcallback(dwDevice_t *dev)
{
  ...
  if (current_mode_trans)
  {
    ...
    {
    case LPS_TWR_POLL:
      // DEBUG_PRINT("%d:send poll\n",selfID);
      poll_tx = departure;
      break;
    case LPS_TWR_FINAL:
      // DEBUG_PRINT("%d:send final\n",selfID);
      final_tx = departure;
      break;
    case LPS_TWR_REPORT + 1:
      // DEBUG_PRINT("%d:send dynamic\n", selfID);
  ...
  else
  {
    ...
    {
    case LPS_TWR_ANSWER:
      // DEBUG_PRINT("%d:send answer\n",selfID);
      answer_tx = departure;
      break;
    case LPS_TWR_REPORT:
      // DEBUG_PRINT("%d:send report\n", selfID);
      break;
    }
  }
}
...
static void rxcallback(dwDevice_t *dev)
{
  ...
  if (current_mode_trans)
  {
    ...
    case LPS_TWR_REPORT:
    {
      ...
      if (calcDist != 0)
      {
        ...
        {
          state.keep_flying = report->keep_flying; // 和0号无人机保持一致
          // DEBUG_PRINT("0de keepflying:%d\n", state.keep_flying);
        }
      ...
      // DEBUG_PRINT("get report transport，%d\n", state.keep_flying);
      dwNewTransmit(dev);
  ...
  else
  {
    ...
    case (LPS_TWR_REPORT + 1):
    {
      ...
      uint8_t rangingID = (uint8_t)(rxPacket.sourceAddress & add2ID) - 1;
      if ((report2->reciprocalDistance) != 0)
      {
        ...
        if (rangingID == 0)
          state.keep_flying = report2->keep_flying; // 也就是说只要0号无人机给我发送了数据包，我的keep_flying就和0号无人机一样了
        state.refresh[rangingID] = true;
      }
      rangingOk = true;
      // DEBUG_PRINT("ranging ok\n");
      uint8_t fromID = (uint8_t)(rxPacket.sourceAddress & add2ID) - 1;
      if (selfID == fromID + 1 || selfID == 0)
      {
        ...
        // DEBUG_PRINT("send-src:%lx,des:%lx\n", txPacket.sourceAddress, txPacket.destAddress);
        dwNewTransmit(dev);
        dwSetDefaults(dev);
        dwSetData(dev, (uint8_t *)&txPacket, MAC802154_HEADER_LENGTH + 2);
        dwWaitForResponse(dev, true);
...
static void twrTagInit(dwDevice_t *dev)
{
  ...
  selfID = (uint8_t)(((configblockGetRadioAddress()) & 0x000000000f) - 1); // 这里做了-修改
  selfAddress = basicAddr + selfID;

  // DEBUG_PRINT("selfID:%d,address:%lx\n", selfID, selfAddress);
  ...
  state.keep_flying = false; // 除了0号无人机，其他无人机一开始的keep_flying都必须为false
  // if (selfID == 0)
  // {
  //   state.keep_flying = true;
  // }
  checkTurn = false;
  rangingOk = false;
}
